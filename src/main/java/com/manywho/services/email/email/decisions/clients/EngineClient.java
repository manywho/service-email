...
public class EngineClient {
    ...
    public String sendResponseToEngine(EmailDecisionRequest emailRequest, String stringResponse) {
        try {
            ...
            /**
             * The sync() call can potentially clear out the invoke response that hangs off of the flowState
             * instance (this seems to typically occur when there are no map elements to move on to). If an
             * invoke response has been generated by the SDK, then grab the state ID from there, if not
             * then just grab it from the incoming request.
             * Being overly cautious here, but am unsure as to whether the a state ID might change when invoking
             * the flows state, for instance with a flow that utilses a child flow (old subflows). So will continue
             * to get the state ID from flowState if possible.
             */
            UUID stateId = flowState.getInvokeResponse() != null ? flowState.getState() : emailRequest.getStateId();

            return runClient.join(emailRequest.getTenantId().toString(), stateId.toString()).execute().body().getJoinFlowUri();
        } catch (IOException e) {
